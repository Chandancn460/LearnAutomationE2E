...
public class ManageCoursesPage {
    ...
    public void deleteCourseByName(String courseName) {
        List<WebElement> rows = wait.until(
                ExpectedConditions.presenceOfAllElementsLocatedBy(By.xpath("//table//tr | //tbody//tr | //div[contains(@class,'course-card')] | //div[contains(@class,'courseCard')]")));
        ...
        for (WebElement row : rows) {
            ...
            if (matchesTokens(normalized, targetNormalized)) {
                List<WebElement> deleteButtons = row.findElements(By.xpath(".//button[normalize-space()='Delete' or contains(@class,'delete') or contains(@aria-label,'delete') or contains(normalize-space(),'Delete')]"));
                    if (!deleteButtons.isEmpty()) {
                        jsClick(deleteButtons.get(0));
